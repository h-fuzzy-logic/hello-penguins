...
class Training(FlowSpec, DatasetMixin):
    ...
    def transform_fold(self):
        ...
        # Add to transform_fold step
        class_counts = train_data['species'].value_counts()
        logging.info(f"Class distribution in training data: {class_counts}")
        logging.info(f"Percentage: {class_counts / len(train_data) * 100}")

        # Visualize with a bar chart
        import matplotlib.pyplot as plt
        import mlflow 

        # Create a single figure with labels
        plt.figure(figsize=(8, 5))
        ax = class_counts.plot(kind='bar')
        plt.title('Species Distribution in Training Data')
        plt.ylabel('Count')
        ...
        plt.tight_layout()
        plt.savefig(f'class_imbalance_fold_{self.fold}.png')
        mlflow.log_artifact(f'class_imbalance_fold_{self.fold}.png')
    ...
    def train(self):
        ...
        with mlflow.start_run(run_id=self.mlflow_run_id):
            ...
            y_true_classes = np.argmax(self.y, axis=1)
            ...
            cm = confusion_matrix(
                ...
                labels=np.arange(len(species_labels))
            )
            
            # Create and log confusion matrix
            self.create_and_log_confusion_matrix(
                cm=cm,
                species_labels=species_labels,
                title='Final Model Confusion Matrix',
                filename_prefix='final_model_confusion_matrix',
                run_id=self.mlflow_run_id
            )
